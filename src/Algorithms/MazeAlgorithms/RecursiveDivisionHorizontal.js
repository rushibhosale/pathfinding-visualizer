let walls = [];
export function RecursiveDivisionHorizontal(board, rowStart, rowEnd, colStart, colEnd, orientation, surroundingWalls = true) {
    ...
    if (orientation === "horizontal") {
        ...
        if (y - 1 - rowStart > colEnd - colStart) {
            RecursiveDivisionHorizontal(board, rowStart, y - 1, colStart, colEnd, orientation, surroundingWalls);
        } else {
            RecursiveDivisionHorizontal(board, rowStart, y - 1, colStart, colEnd, "horizontal", surroundingWalls);
        }
        if (rowEnd - (y + 1) > colEnd - colStart) {
            RecursiveDivisionHorizontal(board, y + 1, rowEnd, colStart, colEnd, orientation, surroundingWalls);
        } else {
            RecursiveDivisionHorizontal(board, y + 1, rowEnd, colStart, colEnd, "vertical", surroundingWalls);
        }
        ...
        if (rowEnd - rowStart > x - 1 - colStart) {
            RecursiveDivisionHorizontal(board, rowStart, rowEnd, colStart, x - 1, "horizontal", surroundingWalls);
        } else {
            RecursiveDivisionHorizontal(board, rowStart, rowEnd, colStart, x - 1, "horizontal", surroundingWalls);
        }
        if (rowEnd - rowStart > colEnd - (x + 1)) {
            RecursiveDivisionHorizontal(board, rowStart, rowEnd, x + 1, colEnd, "horizontal", surroundingWalls);
        } else {
            RecursiveDivisionHorizontal(board, rowStart, rowEnd, x + 1, colEnd, orientation, surroundingWalls);
        }
